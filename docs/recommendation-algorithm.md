...
| `step4` | 进一步计算关联分数 |
| `step5` | 基于**全用户空间**构造图书 0/1 行为向量，并按 Pearson 相关系数公式计算图书相似度 |
| `step6` | 生成用户候选推荐结果 |
...
`step5` 使用 Pearson 相关系数计算图书之间的相似度。系统取**全部用户（全用户空间）**作为向量维度，为每一本图书构造一个 0/1 行为向量：

- 用户对图书产生过浏览、收藏或借阅行为，向量值为 `1`
- 用户未对图书产生行为，向量值为 `0`（**包含"两本图书都未被该用户操作"的 (0,0) 样本点**）
...
其中 `x` 和 `y` 分别表示两本图书在**全用户空间**上的行为向量。由于包含 (0,0) 样本点，皮尔逊系数可正可负：

- 两本书被同一批用户共同偏好、且被其余用户共同忽略时 → **正相关**，相似度高；
- 两本书几乎没有共同用户 → **负相关**，不算相似。

处理说明：两本书的用户集合完全相同时视作完全正相关（系数为 `1.0`）；任一书无行为用户、或某个向量无方差（如被全部用户偏好）时，系数置为 `0`。最终**只保留正相关结果**进入 `step6` 和 `step7`，避免负相关关系参与推荐分数累加。

> 说明：早期实现只在两本书的用户**并集**上构造向量（无 (0,0) 样本点），此时皮尔逊系数数学上恒 ≤ 0，配合"只保留正相关"的过滤会导致推荐链路产出为空。已改回全用户空间构造，使"共同偏好 → 相似度更高"的协同过滤语义成立。
